				...
				<div class="row">
					...
					<?php 
						...
						if(isset($_POST["submit"])){
							$dm=date("Y-m");
							$chk="SELECT * FROM `subscription_fees` WHERE `gn_date` LIKE '$dm%'";
							$check=$db->select($chk);
							if(($check->num_rows)>0){
								echo "<script type='text/javascript'>alert('Already Generated!')</script>";
							}else{
							
							$query ="SELECT `member_code`, `fixed` FROM `members` where Active_status=1";
							...
							while($data=$results->fetch_assoc()){
								...
								//$chk="";
								$ins="INSERT INTO `subscription_fees` 
								(`id`, `members_id`, `invoice_no`, `invoice_date`, 
								`debit_amount`, `credit_amount`, `blance`, 
								`narration`, `paid_id`, `dues_id`, `blance_id`, `gn_date`)
								VALUES (NULL, '$member','0',current_timestamp(),
								'$amount','0','0',
								'Monthly Subsciption Fees','0', '0', '0', current_timestamp())";
