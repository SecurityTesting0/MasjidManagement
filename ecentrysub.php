				...
				<div class="row">	
					...
					<?php 
						if(isset($_POST["submit"])){	
							...
							$sql ="SELECT id, members_id, debit_amount, blance from ecms_fees where members_id='$member_code' order by id DESC LIMIT 0,1";
							...
							if($results==true){ ?>
								...
								Succesfull collect Fees
									...
									<?php 
									$queryvou="SELECT *from ecms_fees ORDER BY id DESC LIMIT 1";
									...
									<button type="button" name="submit"  class="btn btn-primary" data-toggle="modal" data-target=".bd-example-modal-lg">Large modal</button>
									</form> 
								...
								<a href="ec_voucher_print.php?ecvid='<?php echo $vid['id']?>'" class="btn btn-success" target="blank"> Print Money Recpit </a> 
							...
							<?php }else{
								...
								$results=$db->insert($query);
								if($results==true){
									echo"<p class='well text-center' style='margin:0px auto; color:green;'>Succesfull collect Fee</p>";
								}else{
									echo"You should be checked again";
								}
						   }
					...
					?>
						...
						<h4 class="well text-center"><span style="color:red; font-weight:bold;">Collection EC Members Monthly</span> <span style="color:green; font-weight:bold;">  Subscription Fees </span></h4>
