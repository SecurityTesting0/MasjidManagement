				...
				<div class="row">	
					<?php 
						if(isset($_POST["submit"])){	
							...
							$chk="SELECT * FROM `subscription_fees` WHERE `members_id` LIKE '$memac%'";
							$check=$db->select($chk);
							if(($check->num_rows)>0){
								echo "<div class='well text-center' style='background-color:red;'>Balance Already Open </div>";
							}else{
								$query ="INSERT INTO `subscription_fees`(`members_id`, `invoice_no`, `invoice_date`,
					...
					<div class="form-group from_border">
						<h4 class="well text-center"><span style="color:red; font-weight:bold;">Enter General Member Opening</span> <span style="color:green; font-weight:bold;">Balance</span></h4>
									...
									data-dropdown-css-class="select2-purple" name="memac" style="width: 100%;" required>
									 ...
									 <?php		
									  ...
									  <option value="<?php echo $mmcode['member_code'];?> " >
										...
										if($status==0){
											echo $mmcode['member_code'].'-'. $mmcode['First_name'].' '. $mmcode['Last_name'].' (Inactive)';
										}else{
										echo $mmcode['member_code'].'-'. $mmcode['First_name'].' '. $mmcode['Last_name'];
										}
										?>
